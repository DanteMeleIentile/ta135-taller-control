%% --------------------------------------------------------
% GRAFICADOR MULTIPLES RESULTADOS (UNIVERSAL + ESTIMADOS)
% ---------------------------------------------------------
...
ang_matrix    = NaN(muestras_totales, num_ensayos); % Matriz Angulo Estimado
w_matrix      = NaN(muestras_totales, num_ensayos); % Matriz Vel Angular Estimada
v_matrix      = NaN(muestras_totales, num_ensayos); % Matriz Vel Carro Estimada
...
for i = 1:num_ensayos
    ...
    if idx_start > 0 && idx_end <= length(d_real_full)
        ...
        % AQUÍ SE USA LA ESTIMACIÓN DEL OBSERVADOR
        ang_matrix(:, i)    = angle_est_full(idx_start:idx_end); 
        w_matrix(:, i)      = w_est_full(idx_start:idx_end);     
...
ensayos_buenos = [2, 4, 9, 10]; %-10 A
ensayos_buenos = ensayos_buenos(ensayos_buenos <= size(d_real_matrix, 2));

% --- NUEVO: Selección de ensayo a destacar ---
ensayo_destacado = 2; % Elegí acá el número de ensayo que querés resaltar (debe estar en ensayos_buenos)

% Promedios (Solo para Distancia y Vel. del Carro)
...
% FIGURA 1: DISTANCIA (d) - Única que usa medición real cruda (Mantiene promedio)
...
plot(vector_tiempo_ventana, ref_matrix, 'k-', 'LineWidth', 1.5, 'DisplayName', 'Referencia');
plot(vector_tiempo_ventana, d_real_promedio, 'r', 'LineWidth', 2.5, 'DisplayName', 'Prom. Mediciones');
hold off; xlabel('Tiempo [s]'); ylabel('$d(t)$ [cm]', 'Interpreter', 'latex'); xlim([-1 17]); legend('Location', 'best');

%%
% FIGURA 2: ÁNGULO ESTIMADO (theta_hat) - Destaca un ensayo particular
figure('Name', 'Estado x1: Ángulo Estimado', 'Position', [100 250 800 400]);
hold on; grid on;
is_first = true;
for i = ensayos_buenos
    if i == ensayo_destacado
        continue; % Lo salteamos para graficarlo último y que quede por encima
    end
    
    if is_first
        plot(vector_tiempo_ventana, ang_matrix(:, i), 'Color', [0.5, 0.7, 1, 0.9], 'LineWidth', 0.5, 'DisplayName', 'Otras Estimaciones');
        is_first = false;
    else
...
% Graficamos el ensayo destacado
if ismember(ensayo_destacado, ensayos_buenos)
    plot(vector_tiempo_ventana, ang_matrix(:, ensayo_destacado), 'r', 'LineWidth', 2.5, 'DisplayName', ['Ensayo Destacado (', num2str(ensayo_destacado), ')']);
end

if exist('angle_real_sim', 'var')
    plot(t_sim_ajustado(idx_sim), angle_real_sim(idx_sim), 'g-.', 'LineWidth', 2, 'DisplayName', 'Simulación');
end
hold off; xlabel('Tiempo [s]'); ylabel('$\hat{\theta}(t)$', 'Interpreter', 'latex'); xlim([6 12]); legend('Location', 'best');

%%
% FIGURA 3: VELOCIDAD DEL CARRO ESTIMADA (v_hat) - (Mantiene promedio)
figure('Name', 'Estado x4: Velocidad del Carro Estimada', 'Position', [150 200 800 400]);
hold on; grid on;
is_first = true;
for i = ensayos_buenos
    if is_first
        plot(vector_tiempo_ventana, v_matrix(:, i), 'Color', [0.5, 0.7, 1, 0.9], 'LineWidth', 0.5, 'DisplayName', 'Estimaciones (Obs)');
        is_first = false;
    else
...
plot(vector_tiempo_ventana, v_promedio, 'r', 'LineWidth', 2.5, 'DisplayName', 'Prom. Estimaciones');
hold off; xlabel('Tiempo [s]'); ylabel('$\hat{\dot{d}}(t) \ [cm/s]$', 'Interpreter', 'latex'); xlim([4 14]); legend('Location', 'best');

%%
% FIGURA 4: VELOCIDAD ANGULAR ESTIMADA (w_hat) - Destaca un ensayo particular
figure('Name', 'Estado x2: Velocidad Angular Estimada', 'Position', [200 150 800 400]);
hold on; grid on;
is_first = true;
for i = ensayos_buenos
    if i == ensayo_destacado
        continue; % Lo salteamos para graficarlo último
    end
    
    if is_first
        plot(vector_tiempo_ventana, w_matrix(:, i), 'Color', [0.5, 0.7, 1, 0.9], 'LineWidth', 0.5, 'DisplayName', 'Otras Estimaciones');
        is_first = false;
    else
...
% Graficamos el ensayo destacado
if ismember(ensayo_destacado, ensayos_buenos)
    plot(vector_tiempo_ventana, w_matrix(:, ensayo_destacado), 'r', 'LineWidth', 2.5, 'DisplayName', ['Ensayo Destacado (', num2str(ensayo_destacado), ')']);
end

if exist('w_est_sim', 'var')
    plot(t_sim_ajustado(idx_sim), w_est_sim(idx_sim), 'g-.', 'LineWidth', 2, 'DisplayName', 'Simulación');
end
hold off; xlabel('Tiempo [s]'); ylabel('$\hat{\dot{\theta}}(t)$', 'Interpreter', 'latex'); xlim([6 12]); legend('Location', 'best');