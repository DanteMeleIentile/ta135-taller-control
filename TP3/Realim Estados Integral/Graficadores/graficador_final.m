...
% FIGURA 1: DISTANCIA (d) - Única que usa medición real cruda
...
%%
% FIGURA 2: ÁNGULO ESTIMADO (theta_hat)
figure('Name', 'Estado x1: Ángulo Estimado', 'Position', [100 250 800 400]);
hold on; grid on;
is_first = true;
for i = ensayos_buenos
    if is_first
        plot(vector_tiempo_ventana, ang_matrix(:, i), 'Color', [0.5, 0.7, 1, 0.9], 'LineWidth', 0.5, 'DisplayName', 'Estimaciones (Obs)');
        is_first = false;
    else
...
plot(vector_tiempo_ventana, ang_promedio, 'r', 'LineWidth', 2.5, 'DisplayName', 'Prom. Estimaciones');
hold off; xlabel('Tiempo [s]'); ylabel('$\hat{\theta}(t)$', 'Interpreter', 'latex'); xlim([6 12]); legend('Location', 'best');

%%
% FIGURA 3: VELOCIDAD DEL CARRO ESTIMADA (v_hat)
...
% FIGURA 4: VELOCIDAD ANGULAR ESTIMADA (w_hat)
figure('Name', 'Estado x2: Velocidad Angular Estimada', 'Position', [200 150 800 400]);
hold on; grid on;
is_first = true;
for i = ensayos_buenos
    if is_first
        plot(vector_tiempo_ventana, w_matrix(:, i), 'Color', [0.5, 0.7, 1, 0.9], 'LineWidth', 0.5, 'DisplayName', 'Estimaciones (Obs)');
        is_first = false;
    else
...
plot(vector_tiempo_ventana, w_promedio, 'r', 'LineWidth', 2.5, 'DisplayName', 'Prom. Estimaciones');
