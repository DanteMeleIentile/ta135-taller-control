...
u_2     = raw_u_2(indices);
dist_2  = raw_dist_2(indices) - offset_dist_2;

figure;
plot(t_v_2, ang_2, 'b', 'LineWidth', 1.2); 
hold on; 
plot(t_v_2, dist_2, 'g', 'LineWidth', 1.5); 
grid on;
plot(t_v_2, u_2*0.03, 'r', 'LineWidth', 1.5); 
grid on;
...
Y_obs = dist_2(3:end); 
X_reg = [dist_2(2:end-1), dist_2(1:end-2), ang_2(1:end-2)]; % retraso de 2 muestras
...
y_sim_z = lsim(Hz, ang_2, t_v_2);
y_sim_s = lsim(Hs, ang_2, t_v_2);

error = dist_2 - y_sim_s;
MSE = mean(error.^2);
fprintf('\nMSE en ventana: %.4f\n', MSE);


figure();
plot(t_v_2, dist_2, 'g', 'LineWidth', 1.5); hold on;
plot(t_v_2, y_sim_s, 'y--', 'LineWidth', 1.5);
plot(t_v_2, u_2 * 0.03, 'b', 'LineWidth', 1); 
title('Planta Real vs. Modelo Identificado');
legend('Real', 'Estimación', 'Servo');
grid on;